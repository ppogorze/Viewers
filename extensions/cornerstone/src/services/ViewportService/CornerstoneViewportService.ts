...
class CornerstoneViewportService extends PubSubService
  ...
  public setPresentation(
    ...
    console.log('* setPresentation', presentation);
    if (presentation.viewportType === 'stack') {
      const { zoom, pan, properties } = presentation;
      if (zoom && !isNaN(zoom)) {
        console.log('Setting zoom', presentation.id, zoom);
        viewport.setZoom(zoom);
      }
      // if (presentation.pan) viewport.setPan(presentation.pan);
      if (properties) {
        console.log(
          'Restoring VOI range by setProperties',
          presentation.id,
          JSON.stringify(properties),
          JSON.stringify(viewport.getProperties())
        );
  ...
  public setViewportData(
    ...
    console.log('* setViewportData', viewportId, viewportInfo);
  ...
  async _setVolumeViewport(
    ...
    for (const [index, data] of viewportData.data.entries()) {
      ...
      console.log('Adding volume', displaySetInstanceUID);
  ...
  _setDisplaySets(
    ...
    if (viewport instanceof StackViewport) {
      console.log('* Setting stack viewport sets', viewport);
      ...
      console.log('* Setting volume viewport sets', viewport);
